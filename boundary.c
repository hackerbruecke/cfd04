...
static inline int inBounds(int x, int y, int z, int *xlength) {
	return x > 0 && x < xlength[0] + 1 && y > 0 && y < xlength[1] + 1 && z > 0
			&& z < xlength[2] + 1;
}
...
static inline void computeNoSlipCondition(int *xlength, int x, int y, int z,
		int i, int dx, int dy, int dz, double *collideField,
		const int * const flagField) {
	/* Check if the i-th directions is facing the fluid particles and update accordingly */
	if (inBounds(x + dx, y + dy, z + dz, xlength)
			&& flagField[fidx(xlength, x + dx, z + dy, z + dz)] == FLUID) {
		collideField[idx(xlength, x, y, z, i)] = collideField[idx(xlength,
				x + dx, y + dy, z + dz, inv(i))];
	}
}

/* Computed the moving-wall condition for cell (x,y,z)
 * if the respective neighbor determined by i is within bounds and a fluid cell */
static inline void computeMovingWallCondition(int *xlength, int x, int y, int z,
		int i, int dx, int dy, int dz, double *collideField,
		const int * const flagField, const double * const wallVelocity) {
	if (inBounds(x + dx, y + dy, z + dz, xlength)
			&& flagField[fidx(xlength, x + dx, y + dy, z + dz)] == FLUID) {
		double *currentCell = &collideField[idx(xlength, x + dx, y + dy, z + dz,
				0)];
	...
	for (int i = 0; i < Q; ++i) {
		...
		for (int x = 0; x < Nx + 2; ++x) {
			for (int y = 0; y < Ny + 2; ++y) {
				computeNoSlipCondition(xlength, x, y, 0, i, dx, dy, dz,
						collideField, flagField);
				computeMovingWallCondition(xlength, x, y, Nz + 1, i, dx,
						dy, dz, collideField, flagField, wallVelocity);
			}
		}

		for (int x = 0; x < Nx + 2; ++x) {
			for (int z = 0; z < Nz + 2; ++z) {
				computeNoSlipCondition(xlength, x, 0, z, i, dx, dy, dz,
						collideField, flagField);
				computeNoSlipCondition(xlength, x, Ny + 1, z, i, dx, dy,
						dz, collideField, flagField);
			}
		}

		for (int y = 0; y < Ny + 2; ++y) {
			for (int z = 0; z < Nz + 2; ++z) {
				computeNoSlipCondition(xlength, 0, y, z, i, dx, dy, dz,
						collideField, flagField);
				computeNoSlipCondition(xlength, Nx + 1, y, z, i, dx, dy,
						dz, collideField, flagField);
			}
		}
	}
